...
{
    private static final transient Logger LOG = LoggerFactory.getLogger(BookStoreDAOArrayListImpl.class);
    private ArrayList<BookBO> bookBOArrayList = new ArrayList<BookBO>();
    ...
    {
        if(LOG.isDebugEnabled()){LOG.debug("updateBook :: " + bookBO.toString() );}
        if(!bookBOArrayList.contains(bookBO))
        {
        ...
        if(LOG.isDebugEnabled()){LOG.debug("updateBook :: bookList lenght " + bookBOArrayList.size());}
        return true;
    }
    ...
    {
        if(LOG.isDebugEnabled()){LOG.debug("findBookById :: " + id );}
        BookBO bookBO = new BookBO(1L,"If this is a man",1980L);

        if(LOG.isDebugEnabled()){LOG.debug("findBookById :: bookBO " + bookBO.toString());}
        return bookBO;
    }
}
